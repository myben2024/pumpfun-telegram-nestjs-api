...
export class PumpFunService {
  ...
  getMyWallet = async () => {
    ...
    return testAccount.publicKey.toBase58();
  };
  getSOLBalance = async () => {
    if (!this.check_HELIUS_RPC_URL()) return;
    const connection = new Connection(process.env.HELIUS_RPC_URL || '');
    const testAccount = getOrCreateKeypair(KEYS_FOLDER, 'test-account');
    const balance = await connection.getBalance(testAccount.publicKey);
    return balance / LAMPORTS_PER_SOL;
  };
  getTokenBalance = async (mintAddress: PublicKey) => {
    if (!this.check_HELIUS_RPC_URL()) return;
    const connection = new Connection(process.env.HELIUS_RPC_URL || '');
    const testAccount = getOrCreateKeypair(KEYS_FOLDER, 'test-account');
    ...
    return currentSPLBalance;
  };
  buy = async (mintAddress: PublicKey, amount: number) => {
    if (!this.check_HELIUS_RPC_URL()) return;
    const connection = new Connection(process.env.HELIUS_RPC_URL || '');
    const testAccount = getOrCreateKeypair(KEYS_FOLDER, 'test-account');
  ...
  sell = async (mintAddress: PublicKey, amount: number) => {
    if (!this.check_HELIUS_RPC_URL()) return;
    const connection = new Connection(process.env.HELIUS_RPC_URL || '');
    const testAccount = getOrCreateKeypair(KEYS_FOLDER, 'test-account');
